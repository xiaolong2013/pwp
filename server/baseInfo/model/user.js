...
//查询联系人接口
dbOperation.searchUser = function(tag,key,callback){
    var json = {};
    if(tag == 0){
	   json = {$or:[{cellphone:new RegExp(key)},{phone:new RegExp(key)}]};
    }else{
	   json = {$or:[{name:new RegExp(key)},{nickName:new RegExp(key)},{misName:new RegExp(key)}]}
	} 
	conn(function(databaseConnection) {
		databaseConnection.collection('user', function(error, collection) {
			 collection.find(json).toArray(function(error, results) {
				   ...
				   if(error || results.length == 0){
				      callback({"error":0,"msg":"用户名不存在"}); 
				   }else{
				      callback({"error":1,"msg":results});
				   }	
			 });
		});
	});
...
//更改用户的消息接受开关
dbOperation.setMsgConf = function(tag,key,callback){
    var json = {};
    if(tag == 0){
	   json = {$or:[{cellphone:new RegExp(key)},{phone:new RegExp(key)}]};
    }else{
	   json = {$or:[{name:key},{nickName:key},{misName:key}]}
	} 
	conn(function(databaseConnection) {
		databaseConnection.collection('user', function(error, collection) {
			 collection.find(json).toArray(function(error, results) {
